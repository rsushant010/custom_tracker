...
#         CREATE TABLE IF NOT EXISTS user_subjects (
#             id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, subject TEXT, category TEXT,
#             default_daily_target REAL, default_weekly_target REAL,
#             UNIQUE(username, subject)
#         )''')
#     conn.commit()
...
# def add_user_subject(username, subject, category, daily_target, weekly_target):
#     c.execute('INSERT OR IGNORE INTO user_subjects (username, subject, category, default_daily_target, default_weekly_target) VALUES (?,?,?,?,?)', (username, subject, category, daily_target, weekly_target))
#     conn.commit()

# def get_user_subjects(username):
#     c.execute('SELECT id, subject, category, default_daily_target, default_weekly_target FROM user_subjects WHERE username =?', (username,))
#     return pd.DataFrame(c.fetchall(), columns=['id', 'Subject', 'Category', 'Default Daily Target', 'Default Weekly Target'])

# def update_user_subject(subject_id, new_subject, new_category, daily, weekly):
#     c.execute('UPDATE user_subjects SET subject=?, category=?, default_daily_target=?, default_weekly_target=? WHERE id=?', (new_subject, new_category, daily, weekly, subject_id))
#     conn.commit()
...
#     # Use the default weekly target from the user_subjects table
#     df = user_subjects_df[['Subject', 'Default Weekly Target']].rename(columns={'Default Weekly Target': 'Weekly Target'})
#     return df
...
#     # 1. Get the global defaults for all subjects
#     default_targets = user_subjects_df[['Subject', 'Default Daily Target']].set_index('Subject')
    
#     # 2. Get any specific overrides for the selected date
#     c.execute('SELECT subject, daily_target FROM daily_targets WHERE username =? AND target_date =?', (username, target_date))
#     overrides = pd.DataFrame(c.fetchall(), columns=['Subject', 'Daily Target']).set_index('Subject')
    
#     # 3. Update defaults with overrides
#     default_targets.update(overrides)
    
#     return default_targets.reset_index().rename(columns={'Default Daily Target': 'Daily Target'})
...
#             final_subjects = pd.DataFrame(st.session_state.setup_subjects).dropna(how='all').drop_duplicates(subset=['Subject'])
#             for _, row in final_subjects.iterrows():
#                 add_user_subject(username, row['Subject'], row['Category'], row['Daily Target (hrs)'], row['Weekly Target (hrs)'])
